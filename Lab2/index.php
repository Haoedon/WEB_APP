...
<head>
    ...
    <title>Тимошкин Роман Витальевич, Группа 241-352, Лабораторная работа № А-2</title>
...
<body>
    <header>
        ...
        <!-- Заголовок с информацией о студенте и работе -->
        <h1>Тимошкин Роман Витальевич, Лабораторная работа № А-2</h1>
    </header>
    
    <main>
        <?php
        // Инициализация переменных согласно требованиям лабораторной работы
        
        // Начальное значение аргумента функции
        $x = 25;
        
        // Количество вычисляемых значений функции
        $encounting = 30;
        
        // Шаг изменения аргумента
        $step = 2;
        
        // Минимальное значение функции, при котором останавливаются вычисления
        $min_value = -100;
        
        // Максимальное значение функции, при котором останавливаются вычисления
        $max_value = 1000;
        ...
        $i = 0;
        do {
            // Вычисление значения функции в зависимости от значения аргумента x
            if ($x <= 10) {
                // Для x <= 10: f(x) = 10 * x - 5
                $f = 10 * $x - 5;
            } else if ($x < 20) {
                // Для 10 < x < 20: f(x) = (x + 3) * x^2
                $f = ($x + 3) * pow($x, 2);
            } else {
                // Для x >= 20: f(x) = 3 / (x - 25) + 2
                // При x = 25 происходит деление на ноль - обрабатываем эту ситуацию
                if ($x == 25) {
                    $f = 'error';
                } else {
                    $f = 3 / ($x - 25) + 2;
